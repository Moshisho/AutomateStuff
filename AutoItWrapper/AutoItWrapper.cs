...
namespace AutoItWrapper
{
    public static class AutoItWrapper
    {
        ...
        static AutoItWrapper()
        {
            bool is64bit = IntPtr.Size == 8;

            string curDir = Directory.GetCurrentDirectory();
            SetDllDirectory(is64bit ? Path.Combine(curDir, "64") : Path.Combine(curDir, "32"));
        }
        ...
        public static void ClickAnImage(string imgPath) 
        {
            string[] data = UseImageSearch(imgPath);
            int x; int y; int imgWidth; int imgHeight;
            int.TryParse(data[1], out x);
            int.TryParse(data[2], out y);
            int.TryParse(data[3], out imgWidth);
            int.TryParse(data[4], out imgHeight);

            x = (int)(x + (double)imgWidth / 2);
            y = (int)(y + (double)imgHeight / 2);
        ...
        public static int MouseClick(string vsButton, int viX, int viY, int viClicks, int viSpeed)
        {
            ...
            AU3_MouseMove(viX, viY, 10); return AU3_MouseClick(vsButton, viX, viY, viClicks, viSpeed);
        }
        ...
        public static int WinActivate(string vsTitle)
        {
            return AU3_WinActivate(vsTitle, "");
        }
        ...
        public static Position2D WinGetPos(string vsTitle, string vsText)
        {
            Position2D pos;
        ...
        [DllImport("AutoItX3.dll", SetLastError = true, CharSet = CharSet.Auto)]
        static public extern int AU3_WinActivate([MarshalAs(UnmanagedType.LPWStr)]string Title
